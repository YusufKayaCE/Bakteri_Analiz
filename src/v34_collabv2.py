...
USE_AA_KMER    = True
AA_KMER_K      = 4
AA_HASH_BITS   = 12            # [KMER-FIX-2] 14 → 12 (4096 bin, AA için yeterli)
...
def calibrate_and_report(
    ...
    try:
        ...
        fpath = os.path.join(
            REPORTS_DIR, "calibration", f"v29_{safe}_cal.png"
        )
    ...
    for name, clf_fn in [
        ...
        try:
            ...
            model_path = os.path.join(
                MODELS_DIR, f"{safe_name}_{name.lower()}_v29.pkl"
            )
...
def train_and_evaluate(
    ...
    ci_metrics = bootstrap_ci(
        y_te.values, y_pred_prob, final_thr, n_bootstrap=500
    )

    # ── [FIX-4] Kalibrasyon ───────────────────────────────────────────────────
    cal_metrics = calibrate_and_report(
        final_model, X_te, y_te, label_key, final_thr
    )
    ...
    try:
        ...
        imp_df.to_csv(
            os.path.join(SHAP_DIR, f"{safe_key}_shap_v29.csv"), index=False
        )
    except Exception as e:
...
def run_superbug_panel() -> None:
    print("🏥 V29 KLİNİK AMR TAHMİN PANELİ [KMER-FIX + PATH-FIX + KIM 2022]")
    print(
        "   KMER-FIX-1..5: threads backend | chunk-save | 14-bit hash | 60k bp\n"
    ...
    exclude_cols    = [
        "Genome ID", "Bacteria_Type", "Sequence_Type", "ST", "Year", "Country"
    ]
    all_antibiotics = [
    ...
    for species in species_list:
        ...
        for anti in top_antibiotics:
            ...
            feat_cols = [
                c for c in anti_df.columns
                if c not in meta_cols
                and anti_df[c].dtype in [
                    np.int64, np.float64, np.float32, int, float
                ]
            ]
    ...
    print(
        "        Kim 2022 | Per-Species | AA K-mer | SMOTE | "
