...
def _kmer_worker(args):
    """joblib için pickle'lanabilir wrapper — lambda kullanılamaz."""
...
def build_kmer_feature_matrix(genome_ids: list,
    ...
    print(f"\n🧬 [DEĞİŞİKLİK 6] K-mer Matrisi Oluşturuluyor (Hashing Trick)...")
    print(f"   k={k}, hash boyutu={hash_size} (2^{HASH_BITS} bin), "
          f"örnek sayısı={len(genome_ids):,}")
    print(f"   Beklenen bellek: {len(genome_ids) * hash_size * 4 / 1e9:.2f} GB RAM")
    ...
    for idx, row in seq_df.iterrows():
        freq_vec = extract_kmer_freq(row["sequence"], k=k, hash_size=hash_size)
