...
# Google Colab optimize — v36 (v35 + SMOTE-off + adaptif kalibrasyon)
#
# ══════════════════════════════════════════════════════════════════════════════
# v35 → v36 DEĞİŞİKLİKLERİ (veri kaybetmeden AUC/kalibrasyon iyileştirme)
# ──────────────────────────────────────────────────────────────────────────────
# [v36-1] USE_SMOTE=False — sentetik örnek YOK; dengesizlik yalnız
#         scale_pos_weight ile yönetilir. İkili gen uzayında SMOTE kesirli
#         (0.3/0.7) değerler üretip gürültü katıyordu; ayrıca kalibrasyon
#         SMOTE'lu 50/50 prior'a yapılıyordu → olasılıklar yanlış ölçekteydi.
#         Artık kalibrasyon DOĞAL prevalansta → eşik ve VME/ME daha güvenilir.
# [v36-2] Adaptif kalibrasyon: N<1000 → sigmoid (Platt), N≥1000 → isotonic.
#         (Niculescu-Mizil & Caruana 2005: isotonic küçük sette overfit eder.)
# [v36-3] Ayrı çıktı alanı: v36_checkpoint, optuna_v36.db, *_v36.pkl,
#         v36_final_results.csv vb. → v35 çıktılarının ÜZERİNE YAZMAZ.
#         (Girdi önbellekleri — labels, gen cache, PD parquet — PAYLAŞILIR.)
# [v36-NOT] Tüm v35 düzeltmeleri korundu (eşik=test_thr, Host fix, dtype, PD/QRDR).
#
# (Orijinal v35 başlığı aşağıda korunmuştur)
# main_v35_colab.py
# Kim et al. 2022 (Clin Microbiol Rev 35(3):e00179-21) Tam Hizalama
...
CHECKPOINT_FILE = os.path.join(REPORTS_DIR, "v36_checkpoint.csv")
OPTUNA_DB_FILE  = "sqlite:///" + os.path.abspath(MODELS_DIR) + "/optuna_v36.db"
...
# ── SMOTE ─────────────────────────────────────────────────────────────────────
# [v36-1] SMOTE KAPALI. Dengesizlik scale_pos_weight ile yönetilir (SMOTE'siz
# dalda pw_upper = max(4, natural_ratio*1.5) → güçlü pozitif ağırlık). Sentetik
# gen-vektörü gürültüsü yok + kalibrasyon doğal prevalansta → daha temiz olasılık.
USE_SMOTE        = False
SMOTE_MIN_RATIO  = 2.0
...
def veri_bias_raporu(y_df: pd.DataFrame) -> None:
    ...
    for col, label in [
        ...
        out  = os.path.join(
            REPORTS_DIR, "bias_reports", f"v36_bias_{col.lower()}.csv"
        )
...
def optimize_hyperparameters(
    ...
    study_name = f"study_v36_{safe_key}"
...
def train_mic_regression_model(
    ...
    try:
        ...
        joblib.dump(
            ...
            os.path.join(MODELS_DIR, f"{safe}_mic_v36.pkl"),
        )
        ...
        out_df.to_csv(
            os.path.join(REPORTS_DIR, "mic_reports", f"v36_{safe}_mic.csv"),
            index=False,
        )
        # Klinik raporlama
...
def calibrate_and_report(
    ...
    try:
        ...
        fpath = os.path.join(
            REPORTS_DIR, "calibration", f"v36_{safe}_cal.png"
        )
    ...
    for name, clf_fn in [
        ...
        try:
            ...
            model_path = os.path.join(
                MODELS_DIR, f"{safe_name}_{name.lower()}_v36.pkl"
            )
...
def train_and_evaluate(
    ...
    # CalibratedClassifierCV cv=3 training fold içinde fit (X_te'ye dokunmaz).
    # [v36-2] Adaptif yöntem: küçük örnekte isotonic overfit eder
    # (Niculescu-Mizil 2005) → N<1000'de sigmoid (Platt), aksi halde isotonic.
    cal_in_pipeline = False
    _cal_method = "isotonic" if len(y_tr) >= 1000 else "sigmoid"
    try:
    ...
    try:
        ...
        lr_path    = os.path.join(MODELS_DIR, f"{safe_bname}_lr_v36.pkl")
        rf_path    = os.path.join(MODELS_DIR, f"{safe_bname}_rf_v36.pkl")
    ...
    try:
        ...
        imp_df.to_csv(
            os.path.join(SHAP_DIR, f"{safe_key}_shap_v36.csv"), index=False
        )
    except Exception as e:
    ...
    # ── Model Kaydet ──────────────────────────────────────────────────────────
    safe_key   = re.sub(r"[^\w\-]", "_", label_key.lower())
    model_path = os.path.join(MODELS_DIR, f"{safe_key}_v36.pkl")
    joblib.dump(
        {
...
def run_superbug_panel(
    ...
    None = hepsi.
    """
    print("🏥 V36 KLİNİK AMR TAHMİN PANELİ [v35 + SMOTE-off + adaptif kalibrasyon]")
    print(
    ...
    try:
        ...
        for _, row in results_df.iterrows():
            ...
            shap_csv = os.path.join(SHAP_DIR, f"{safe_key}_shap_v36.csv")
    ...
    report_path = os.path.join(REPORTS_DIR, "v36_final_results.csv")
