...
def _detect_device() -> str:
    """CUDA kullanılabilir mi kontrol et; değilse CPU'ya düş."""
    try:
        import subprocess
...
OPTUNA_TRIALS     = 25
# GPU modunda n_jobs XGBoost tarafından görmezden gelinir;
# CPU modunda paralel CV deadlock yaratır → her iki durumda da 1 kullan.
OPTUNA_N_JOBS     = 1
# Nihai model: GPU varsa cihaz zaten "cuda", yoksa tüm CPU çekirdekleri.
...
def fetch_amr_genes_from_bvbrc(genome_ids):
    ...
    if os.path.exists(CACHE_FILE):
        # BUG DÜZELTMESİ #4: Yarım/bozuk cache dosyasını yakala
        try:
    ...
    print(f"🌐 V17 TEMİZ BAŞLANGIÇ: Cache bulunamadı. "
          f"{len(genome_ids):,} bakteri için API'den genler çekiliyor...")
    print("⚠️  Bu işlem saatler sürebilir. Arkanı yaslan ve bilgisayarını açık bırak.")
...
def optimize_hyperparameters(X_train, y_train, pos_weight):
    """
    BUG DÜZELTMESİ #3: cross_val_score içinde n_jobs=OPTUNA_N_JOBS (1) kullanılıyor.
    Optuna kendi döngüsünü seri çalıştırır; iç CV paralelliği deadlock yaratır.
    """
    def objective(trial):
        param = {
            ...
            'verbosity'        : 0,
            # GPU modunda n_jobs XGBoost tarafından görmezden gelinir;
            # deadlock riski olmaz ama CPU modu için yine 1 bırakıyoruz.
            'n_jobs'           : OPTUNA_N_JOBS,
            'device'           : DEVICE,   # ← "cuda" veya "cpu"
        }
        cv     = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)
        model  = xgb.XGBClassifier(**param)
        # GPU modunda cross_val_score n_jobs=1 olmalı — GPU zaten paraleldir.
    ...
    best_params.update({
        ...
        'verbosity'       : 0,
        'n_jobs'          : MODEL_N_JOBS,
        'device'          : DEVICE,   # ← nihai model de aynı cihazda
    })
...
# ── 3. Model Eğitimi ──────────────────────────────────────────────────────────
def train_and_evaluate(X_gene, y_all, antibiotic_name, gene_cols):
    """
    BUG DÜZELTMESİ #1 & #2: gene_cols artık dışarıdan parametre olarak alınıyor
    (merge sonrası hesaplanmış, güncel liste). X_te.reindex() train_test_split
    SONRASINDA yapılıyor; böylece train kolon sırası referans alınıyor.
    """
    ...
    # BUG DÜZELTMESİ #6: Boş gene_cols durumunda erken çıkış
    if not gene_cols:
        print(f"   ⚠️  Gen sütunu bulunamadı, {antibiotic_name} atlanıyor.")
        return None
    ...
    # BUG DÜZELTMESİ #1: reindex, split SONRASINDA train kolonlarına göre yapılıyor
    ...
    # BUG DÜZELTMESİ #5: confusion_matrix 2x2 değilse güvenli çözüm
    cm = confusion_matrix(y_te, y_pred_opt)
    if cm.shape == (2, 2):
        tn, fp, fn, tp = cm.ravel()
    else:
        # Tek sınıf kaldıysa (tüm tahminler aynı) uyar ve sıfır ata
        print(f"   ⚠️  Confusion matrix 2x2 değil (shape={cm.shape}). "
              "Test setinde tek sınıf mı var? TP/FN sıfır kabul edildi.")
        tn, fp, fn, tp = 0, 0, 0, 0
    ...
    model_path = os.path.join(MODELS_DIR, f"{safe_name}_v17.pkl")
    joblib.dump({
        "model"      : final_model,
        "threshold"  : best_thr,
...
def run_superbug_panel():
    print("🏥 V17.0 KLİNİK SÜPER BAKTERİ PANELİ (TEMİZ BAŞLANGIÇ & OPTUNA)\n")
    ...
    if genes_df.empty:
        print("❌ Gen verisi boş, işlem durduruluyor.")
        return
    ...
    # BUG DÜZELTMESİ #2: gene_cols, merge'den ÖNCE X_matrix'ten alınmalı
    # (merge sonrası label kolonları da karışır)
    X_matrix.columns = [re.sub(r"[\[\]<>]", "", str(c)) for c in X_matrix.columns]
    X_matrix["Genome ID"] = X_matrix["Genome ID"].astype(str).str.strip()

    # gene_cols burada doğru şekilde hesaplanıyor — sadece gen sütunları
    gene_cols = [c for c in X_matrix.columns if c != "Genome ID"]

    if not gene_cols:
        print("❌ Pivot sonrası gen sütunu bulunamadı, işlem durduruluyor.")
        return

    # Right join: etiketi olan ama gen verisi olmayan bakterileri de koru
    final_df = pd.merge(X_matrix, y_df, on="Genome ID", how="right")

    # Merge sonrası eksik gen değerlerini 0 ile doldur
    ...
    for anti in top_antibiotics:
        ...
        if len(anti_df) < 50:
            print(f"⚠️  {anti}: yeterli etiketli örnek yok (<50), atlanıyor.")
            continue

        X_gene = anti_df[gene_cols]
        y_all  = anti_df[anti].astype(int)

        # BUG DÜZELTMESİ #2: gene_cols artık fonksiyona parametre olarak geçiliyor
        row = train_and_evaluate(X_gene, y_all, anti, gene_cols)
        if row:
            results.append(row)

    if not results:
        print("\n❌ Hiçbir antibiyotik için model eğitilemedi.")
        return

    print("\n" + "=" * 80)
    print("         V17.0 FİNAL PERFORMANS TABLOSU (OPTUNA OPTİMİZE)")
    print("=" * 80)
    results_df = pd.DataFrame(results)
    print(results_df.to_string(index=False))

    report_path = os.path.join(REPORTS_DIR, "v17_final_results.csv")
