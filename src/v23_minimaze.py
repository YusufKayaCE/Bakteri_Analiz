# main_v23.py
# Değişiklikler (v22 → v23):
#   1. KALİBRASYON KALDIRILDI — Isotonic IR train setiyle fit edildiğinde
#      büyük veri setlerinde tüm olasılıkları 0 veya 1'e itiyor.
#      Ampicillin'de TN=0 bunun sonucuydu (tüm olasılıklar 1'e yaklaştı).
#      Çözüm: ham XGBoost olasılıkları kullanılıyor; XGBoost'un kendi
#      olasılık kalibrasyonu (logloss hedefi) zaten yeterince iyi.
#   2. pos_weight üst sınırı GENİŞLETİLDİ — Gentamicin ve Ciprofloxacin
#      için Optuna natural_ratio * 0.6 ile sınırlandırılıyordu.
#      Gentamicin imbalance=5x → üst sınır 3.0 iken Recall=0.21 kaldı.
#      Yeni üst sınır: natural_ratio * 1.2, minimum 3.0.
#   3. BAKTERİ TÜRÜ MIN_FREQ FİLTRESİ — Ampicillin için sadece 1 bakteri
#      türü sütunu eklenmişti (bact_SALMONELLA). Tek sınıf dominant olunca
#      model bunu direnç ile ilişkilendiriyor. Yeni kural: veri setinin
#      en az %5'ini oluşturan türler dahil edilir, geri kalanlar "Other".
#   4. CV fold kalibrasyon bloğu da temizlendi — artık ham prob kullanılıyor.
#   5. Yeni Optuna DB (v23).
...
# v23: bakteri türü minimum frekans eşiği
BACT_MIN_FREQ    = 0.05   # Veri setinin en az %5'ini oluşturan türler dahil edilir
...
OPTUNA_DB_FILE = f"sqlite:///{os.path.abspath(MODELS_DIR)}/optuna_studies_v23.db"
...
# ── 2. Bakteri Türü One-Hot Encoding (Min-Freq Filtreli) ──────────────────────
def add_bacteria_type_features(X_gene: pd.DataFrame,
                                bacteria_series: pd.Series,
                                min_freq: float = BACT_MIN_FREQ) -> pd.DataFrame:
    """
    v23 DÜZELTMESİ: Minimum frekans filtresi eklendi.

    V22 sorunu:
    - Ampicillin'de sadece 1 bakteri türü sütunu vardı (bact_SALMONELLA).
    - Salmonella bu veri setinde neredeyse hep ampisilin dirençliydi.
    - Model TEM beta-laktamaz yerine bact_SALMONELLA'yı birincil feature
      olarak kullanmaya başladı (SHAP grafiğinde en tepede görünüyordu).
    - TN=0 sonucu: model her Salmonella'yı dirençli işaretledi.

    Çözüm: veri setinin en az %5'ini oluşturan türler dahil edilir,
    geri kalanlar "Other" kategorisine toplanır.
    Tek bir tür %95'ten fazlaysa hiçbir bakteri özelliği eklenmez
    (anlamsız gürültü olur).
    """
    ...
    # Min-freq eşiğini geçen türler
    valid_types = freq[freq >= min_freq].index.tolist()

    if len(valid_types) <= 1:
        # Tek dominant tür → özellik ekleme (gürültü olur)
        print(f"   ⚠️  Bakteri türü tek dominanttır (≥{min_freq*100:.0f}% eşiği geçen "
              f"{len(valid_types)} tür) → bakteri özelliği eklenmedi.")
        return X_gene

    # Küçük türleri "Other" olarak etiketle
...
# ── 3. Eşik Seçimi: 4 Kademeli Tolerans Sistemi ──────────────────────────────
def _select_threshold(y_true, y_prob,
    ...
    """
    4 kademeli öncelik sistemi:

    Öncelik 1 — MÜKEMMEL (full):
        Recall >= recall_min (0.80) VE Spec >= spec_min (0.50)
        → En yüksek F1'i seç

    Öncelik 2 — TOLERANS (tolerance):
        Recall >= recall_min - recall_tolerance (0.70) VE Spec >= spec_min
        → Recall biraz esnedi ama Specificity korunuyor
        → Ayrı değişken: eski kodu ezemez

    Öncelik 3 — RECALL ONLY (recall_only):
        Recall >= recall_min, Spec umursamaz
        → Ayrı değişken
    ...
    best_thr = None;  best_f1  = -1   # Öncelik 1
    tol_thr  = None;  tol_f1   = -1   # Öncelik 2
    rec_thr  = None;  rec_f1   = -1   # Öncelik 3
...
# ── 4. Optuna: pos_weight Genişletilmiş Aralıkla ─────────────────────────────
def optimize_hyperparameters(X_train, y_train, antibiotic_name):
    """
    v23 DÜZELTMESİ: pos_weight üst sınırı genişletildi.

    V22 sorunu:
    - pos_weight üst sınırı = natural_ratio * 0.6 idi.
    - Gentamicin natural_ratio ≈ 5 → üst sınır = 3.0
    - Optuna 2.66 buldu ama Recall hâlâ 0.21 kaldı.
    - Sorun: Optuna'nın skoru 0.4*AP + 0.6*Recall → Recall düşükken
      AP yüksekti, Optuna bunu "iyi" zannetmeye devam etti.

    V23 düzeltmesi:
    - pos_weight üst sınırı = natural_ratio * 1.2, min 3.0
    - Optuna skoru: eğer recall_mean < 0.70 ise ceza uygula
      score = recall_mean * 0.7 + ap_mean * 0.3 - penalty
    """
    n_neg = (y_train == 0).sum()
    n_pos = (y_train == 1).sum()
    natural_ratio = min(n_neg / (n_pos + 1e-9), 10.0)

    # v23: üst sınır genişletildi
    pw_upper = max(3.0, natural_ratio * 1.2)

    study_name   = f"study_v23_{antibiotic_name.lower()}"
    ...
    def objective(trial):
        ...
        # v23: Recall < 0.70 olursa ağır ceza — Optuna bunu kaçınır
...
def train_and_evaluate(X_features, y_all, antibiotic_name):
    """
    v23: Kalibrasyon yok — ham XGBoost olasılıkları kullanılıyor.
    XGBoost logloss hedefiyle eğitildiğinde olasılıkları zaten
    iyi kalibre edilmiştir; isotonic IR büyük veri setlerinde
    olasılıkları 0/1'e itiyor (Ampicillin TN=0 bunun sonucuydu).
    """
    ...
    # ── CV: fold bazlı eşik seçimi (kalibrasyonsuz) ───────────────────────────
    ...
    for fold_tr_idx, fold_val_idx in skf.split(X_tr, y_tr):
        ...
        # v23: ham olasılıklar — kalibrasyon yok
    ...
    # v23: kalibrasyon yok — ham olasılıklar
    ...
    # CV median + test eşiği ağırlıklı ortalama
    ...
    # Klinik hazır: tam veya tolerans modu
    ...
    model_path = os.path.join(MODELS_DIR, f"{safe_name}_v23.pkl")

    joblib.dump({
        "model"        : final_model,   # reporting_module bu key'i arıyor
        "pipeline"     : final_model,   # geriye dönük uyumluluk
...
def run_superbug_panel():
    print("🏥 V23 KLİNİK SÜPER BAKTERİ PANELİ")
    print("   (Kalibrasyonsuz + Genişletilmiş pos_weight + Bakteri Min-Freq Filtresi)\n")
    ...
    for anti in top_antibiotics:
        ...
        # v23: min-freq filtreli bakteri türü ekleme
        if has_bacteria_type:
    ...
    print("\n" + "=" * 105)
    print("        V23 FİNAL PERFORMANS TABLOSU")
    print("        (Kalibrasyonsuz + Genişletilmiş pos_weight + Bakteri Min-Freq Filtresi)")
    print("=" * 105)
    ...
    report_path = os.path.join(REPORTS_DIR, "v23_final_results.csv")
