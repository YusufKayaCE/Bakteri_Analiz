...
#   - _kmer_worker() yardımcı fonksiyonu eklendi
#
# DÜZELTMELER (v26.2 → v26.3):
#   - remove_correlated_features(): K-mer sütunlarına Pearson korelasyonu UYGULANMIYOR.
#
# DÜZELTMELER (v26.3 → v26.4):
#   - optimize_hyperparameters(): tree_method='hist' eklendi.
#   - max_bin=256 eklendi.
#   - colsample_bytree arama aralığı 0.6-1.0 → 0.3-0.6 daraltıldı.
#
# DÜZELTMELER (v26.4 → v26.5):  ← YENİ
#   - GPU → CPU Dinamik Fallback eklendi.
#   - FALLBACK_TO_CPU global bayrağı: İlk OOM sonrası tüm denemeler CPU'da çalışır.
#   - Hata zinciri taraması (__cause__, __context__) ile sarmalanmış OOM hataları
#     güvenilir şekilde yakalanır (sklearn cross_validate sarmalama sorununu çözer).
#   - CPU da OOM yediyse optuna.exceptions.TrialPruned() → program çökmez, devam eder.
#   - Her antibiyotik başında FALLBACK_TO_CPU sıfırlanır (ilaçlar arası sızma önlenir).
#   - colsample_bytree aralığı 0.3-0.6 → 0.15-0.25 (ek VRAM tasarrufu, GPU'ya son şans).
#   - n_estimators üst sınırı 400 → 200 (VRAM ve RAM tasarrufu).
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# [DEĞİŞİKLİK v26.5] GPU → CPU FALLBACK BAYRAGI
# ══════════════════════════════════════════════════════════════════════════════
# False  → Denemeler DEVICE (GPU/CPU) üzerinde çalışır.
# True   → GPU'da OOM alındı; kalan TÜM denemeler CPU'da çalışır.
# Her antibiyotik başında optimize_hyperparameters() içinde sıfırlanır.
FALLBACK_TO_CPU: bool = False

# OOM tespiti için aranacak anahtar kelimeler (hata zinciri dahil)
_OOM_KEYWORDS = [
    "out of memory",
    "cudaerrormemoryallocation",
    "memory allocation error",
    "free memory: 0b",          # logda tam bu format görüldü
]

def _is_oom_error(exc: Exception) -> bool:
    """
    sklearn'ın cross_validate'i XGBoostError'ı ValueError içine sardığından
    yalnızca str(e) bakmak yetmez. __cause__ ve __context__ zinciri de taranır.
    """
    chain_text = " ".join([
        str(exc).lower(),
    ...
    return any(kw in chain_text for kw in _OOM_KEYWORDS)
...
# ══════════════════════════════════════════════════════════════════════════════
# ── 7. Optuna — GPU → CPU Dinamik Fallback [DEĞİŞİKLİK v26.5]
# ══════════════════════════════════════════════════════════════════════════════
def optimize_hyperparameters(X_train, y_train, antibiotic_name):
    # ── [v26.5] Her antibiyotik için bayrağı sıfırla ─────────────────────────
    # Ciprofloxacin OOM aldıysa Amoxicillin GPU'yu denemeden geçmesin diye
    # sıfırlıyoruz. Eğer GPU gerçekten yetersizse zaten ilk trial'da tekrar
    # OOM alır ve bayrak yeniden True'ya döner. Ek maliyet: yalnızca 1 OOM.
    ...
    def objective(trial):
        ...
        # ── [v26.5] Cihaz seçimi: bayrak True ise direkt CPU ─────────────────
        current_device = 'cpu' if FALLBACK_TO_CPU else DEVICE
        current_njobs  = -1    if FALLBACK_TO_CPU else OPTUNA_N_JOBS

        params = {
            # [v26.5] n_estimators üst sınırı 400 → 200: hem VRAM hem RAM tasarrufu
            'n_estimators'    : trial.suggest_int('n_estimators', 100, 200, step=50),
            'max_depth'       : trial.suggest_int('max_depth', 3, 7),
            'learning_rate'   : trial.suggest_float('learning_rate', 0.01, 0.2, log=True),
            'subsample'       : trial.suggest_float('subsample', 0.6, 1.0),
            # [v26.5] colsample_bytree 0.3-0.6 → 0.15-0.25:
            # GPU'ya yüklenen sütun sayısını ~yarıya indirir, GPU'ya son şans tanır.
        ...
        try:
            ...
            # ── [v26.5] OOM Tespiti: hata zincirini tara ─────────────────────
            # sklearn cross_validate, XGBoostError'ı ValueError içine sardığından
            # yalnızca str(e) bakmak güvenilir değil. __cause__ ve __context__
            # zinciri de taranarak sarmalanmış OOM mesajları yakalanır.
            if _is_oom_error(e):
                if not FALLBACK_TO_CPU:
                    print(f"\n   ⚠️  GPU OOM (VRAM Doldu) → "
                          f"Trial {trial.number}'dan itibaren CPU'ya devrediliyor!")
                    FALLBACK_TO_CPU = True

                # Bu başarısız trial'ı CPU ile kurtar
                ...
                try:
                    ...
                    # CPU da OOM yediyse (16 GB RAM doldu) → Optuna'ya "atla" de
                    # Program çökmez, kalan trial'lar devam eder.
                    if _is_oom_error(cpu_exc):
                        print(f"\n   🚨 CPU da OOM! (RAM yetersiz) → "
                              f"Trial {trial.number} atlanıyor (TrialPruned).")
                        raise optuna.exceptions.TrialPruned()
                    raise  # OOM dışı CPU hatası → normal şekilde ilet
            else:
                raise  # OOM dışı GPU hatası → normal şekilde ilet
    ...
    # ── [v26.5] Final model cihazı: OOM yaşandıysa CPU kullan ────────────────
    final_device  = 'cpu' if FALLBACK_TO_CPU else DEVICE
    final_n_jobs  = -1    if FALLBACK_TO_CPU else MODEL_N_JOBS
    best_params.update({
        'tree_method' : 'hist',
        'max_bin'     : 256,
        'eval_metric' : 'logloss',
