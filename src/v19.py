# main_v19.py
# Değişiklikler (v18 → v19):
#   1. SMOTE ile sınıf dengesizliği giderimi
#   2. Eşik seçimine Specificity kısıtı eklendi
#   3. Optuna metriği: average_precision + f1 harmonik ortalaması
#   4. Sonuç tablosuna Specificity ve FP Sayısı eklendi
...
SPECIFICITY_MIN  = 0.50   # ← YENİ: minimum kabul edilebilir specificity
OPTUNA_TRIALS    = 25
OPTUNA_N_JOBS    = 1
...
def optimize_hyperparameters(X_train, y_train, pos_weight, antibiotic_name):
    """
    v19 DEĞİŞİKLİĞİ: Sadece average_precision yerine
    average_precision + f1 harmonik ortalaması optimize ediliyor.
    Bu sayede model hem recall hem precision'ı dengeli öğreniyor.
    pos_weight: SMOTE sonrası 1.0 geliyor, sınıflar zaten dengeli.
    """
    study_name = f"study_v19_{antibiotic_name.lower()}"
...
def train_and_evaluate(X_gene, y_all, antibiotic_name):
    ...
    # ── YENİ: SMOTE ile dengeleme ─────────────────────────────────────────────
    # k_neighbors: dirençli örnek sayısı 5'ten azsa güvenli şekilde ayarla
    k_neighbors = min(5, n_resistant - 1)
    if k_neighbors < 1:
        print(f"   ⚠️  Dirençli örnek sayısı SMOTE için çok az, atlanıyor.")
        ...
        smote_applied = True
        after = pd.Series(y_tr_resampled).value_counts().to_dict()
        print(f"   ⚖️  SMOTE uygulandı → Duyarlı(0): {after.get(0,0)} | "
              f"Dirençli(1): {after.get(1,0)}")

    # SMOTE sonrası sınıflar dengeli → pos_weight = 1.0
    pos_weight = 1.0 if smote_applied else (n_susceptible / max(n_resistant, 1))
    # ─────────────────────────────────────────────────────────────────────────

    best_xgb_params = optimize_hyperparameters(
        X_tr_resampled, y_tr_resampled, pos_weight, antibiotic_name
    )
    ...
    for fold_tr_idx, fold_val_idx in skf.split(X_tr_resampled, y_tr_resampled):
        X_cv_tr  = X_tr_resampled.iloc[fold_tr_idx]
        X_cv_val = X_tr_resampled.iloc[fold_val_idx]
        y_cv_tr  = y_tr_resampled.iloc[fold_tr_idx]
        y_cv_val = y_tr_resampled.iloc[fold_val_idx]
        ...
        # ── YENİ: Hem recall hem precision (specificity proxy) kısıtı ────────
        valid_fold = np.where(
            (r_fold[:-1] >= RECALL_THRESHOLD) &
            (p_fold[:-1] >= SPECIFICITY_MIN)
        )[0]

        if len(valid_fold) == 0:
            # Sadece recall şartını dene, specificity'i esnelt
            valid_fold = np.where(r_fold[:-1] >= RECALL_THRESHOLD)[0]
            if len(valid_fold) > 0:
                print(f"   ⚠️  Bu fold'da precision >= {SPECIFICITY_MIN} sağlanamadı, "
                      f"sadece recall hedeflendi.")
        # ─────────────────────────────────────────────────────────────────────
    ...
    # Final model: orijinal (SMOTE'suz) test seti üzerinde değerlendiriliyor
    # ama SMOTE'lu eğitim verisiyle eğitiliyor
    final_model = xgb.XGBClassifier(**best_xgb_params)
    final_model.fit(X_tr_resampled, y_tr_resampled)
    ...
    if cm.shape == (2, 2):
        ...
        specificity = tn / (tn + fp + 1e-9)   # ← YENİ
    else:
        tn, fp, fn, tp = 0, 0, 0, 0
        specificity = 0.0
    ...
    print(f"   Test F1       : {test_f1:.3f}  |  Test Recall: {test_recall:.3f}  "
          f"|  AUC: {test_auc:.3f}")
    print(f"   Specificity   : {specificity:.3f}  |  FP: {fp}  |  FN: {fn}")  # ← YENİ

    # ── YENİ: Klinik hazırlık → hem recall hem specificity şartı ─────────────
    ...
    model_path = os.path.join(MODELS_DIR, f"{safe_name}_v19.pkl")

    joblib.dump({
        "model"        : final_model,
        "threshold"    : best_thr,
        "train_cols"   : X_tr_resampled.columns.tolist(),
    ...
    return {
        ...
        "Specificity"  : f"{specificity:.3f}",    # ← YENİ
        "AUC-ROC"      : f"{test_auc:.3f}",
        "TP / FN"      : f"{tp} / {fn}",
        "FP Sayısı"    : int(fp),                  # ← YENİ
        "SMOTE"        : "✅" if smote_applied else "—",
        "Tutarlılık"   : gap_flag,
...
def run_superbug_panel():
    print("🏥 V19 KLİNİK SÜPER BAKTERİ PANELİ (SMOTE + SHAP + OPTUNA + RAPORLAMA)\n")
    ...
    print("\n" + "=" * 90)
    print("        V19 FİNAL PERFORMANS TABLOSU (SMOTE + OPTUNA + SHAP)")
    print("=" * 90)
    ...
    report_path = os.path.join(REPORTS_DIR, "v19_final_results.csv")
