...
# ===== YENİ EKLENEN MODÜL ENTEGRASYONU =====
# Raporlama modülümüzü projenin içine bir kütüphane gibi dahil ediyoruz
...
# Dizinleri önce oluştur, sonra OPTUNA_DB_FILE'ı tanımla
os.makedirs(MODELS_DIR, exist_ok=True)
os.makedirs(REPORTS_DIR, exist_ok=True)
os.makedirs(SHAP_DIR, exist_ok=True)
os.makedirs(os.path.join(REPORTS_DIR, "figures"), exist_ok=True) # Grafikler için klasör
...
def train_and_evaluate(X_gene, y_all, antibiotic_name):
    ...
    for fold_tr_idx, fold_val_idx in skf.split(X_tr, y_tr):
        ...
        if len(valid_fold) > 0:
            ...
            fold_thr = 0.5
            print(f"   ⚠️  Bu fold'da recall >= {RECALL_THRESHOLD} sağlanamadı, threshold=0.5 kullanıldı.")
    ...
    # ===== YENİ EKLENEN ÇİZİM MOTORU TETİKLEYİCİSİ =====
    # Model ve threshold hazır, test verimiz elimizde. Hemen grafikleri çizdiriyoruz.
...
def run_superbug_panel():
    print("🏥 V18.1 KLİNİK SÜPER BAKTERİ PANELİ (SHAP + OPTUNA + RAPORLAMA)\n")
