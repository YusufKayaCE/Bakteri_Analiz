...
def fetch_genome_sequences(genome_ids: list) -> pd.DataFrame:
    ...
    for b_idx, i in enumerate(range(0, len(missing_ids), SEQ_BATCH_SIZE)):
        ...
        # Bu adım ArrowMemoryError'ı tamamen önler.
        # ────────────────────────────────────────────────────────────────────
    ...
    if all_rows:
        ...
        # Tek birleştirme (new_df küçük, cached_df zaten disk'ten okunmuş)
        if cached_df.empty:
            final_seq_df = new_df
        else:
            final_seq_df = pd.concat([cached_df, new_df], ignore_index=True)

        # Sadece bu projeye ait genomları tut
        final_seq_df = final_seq_df[
            final_seq_df["Genome ID"].isin(genome_ids)
        ].reset_index(drop=True)

        # ── DÜZELTME: chunk'lı parquet yazımı ────────────────────────────────
        # row_group_size=500 → PyArrow küçük parçalar halinde tahsis eder,
        # 3 GB'lık tek realloc yerine çok sayıda küçük alloc → hata önlenir.
        # ────────────────────────────────────────────────────────────────────
        try:
...
def build_kmer_feature_matrix(genome_ids: list,
    ...
    # PyArrow ile devasa dosyayı parça parça (chunk) okuyoruz (RAM koruması)
    parquet_file = pq.ParquetFile(SEQ_CACHE_FILE)
    
    all_rows_matrix = []
    all_gids = []
    
    # Dosyayı 1000 satırlık parçalar halinde oku
    for i, batch in enumerate(parquet_file.iter_batches(batch_size=1000)):
        chunk_df = batch.to_pandas()
        
        # O parçadaki (1000 adet) genom için paralel hesaplama yap
        args_list = [
            (row["sequence"], k, hash_size)
            for _, row in chunk_df.iterrows()
        ]
        
        # Parallel işlemi sadece o 1000 satır için çalıştır
    ...
    # Artık RAM'de sadece sayısal matrisler var, dev metinler yok!
...
# Sebep: 32.768 k-mer sütunu için korelasyon matrisi hesaplamak saatler sürer
# ve ~5 GB RAM tüketir. K-mer sütunları zaten build_kmer_feature_matrix içinde
# VarianceThreshold ile temizlenmiştir; ek korelasyon filtresi gerekmez.
def remove_correlated_features(X: pd.DataFrame,
    ...
    # Sadece gen sütunlarına korelasyon uygula
    if len(gene_cols) >= 2:
    ...
    # K-mer sütunlarına korelasyon UYGULANMIYOR
    # (32.768 sütun × Pearson → RAM taşması + saatlik bekleme)
    # VarianceThreshold zaten build_kmer_feature_matrix'te uygulandı.
